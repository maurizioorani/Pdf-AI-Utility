...
CREATE TABLE IF NOT EXISTS document_chunk (
    id BIGSERIAL PRIMARY KEY, -- Reverted to BIGSERIAL for PostgreSQL
    ...
    embedding VECTOR(384), -- Reverted to VECTOR(384) for PostgreSQL
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);

-- Removed separate document_chunk_embedding table as embedding is now a column in document_chunk
...
COMMENT ON COLUMN document_chunk.embedding IS 'Vector embedding of the content for semantic search'; -- Added comment for embedding
